...
int getOrientZ() {
  imu::Vector<3> euler = oriSensor.getVector(Adafruit_BNO055::VECTOR_EULER);
  int returnVal = euler.z();
  pdebug.print("Z: ");
  pdebug.println(returnVal);
  return returnVal;
}

boolean is_RampeUp() {
  if(getOrientZ() - startOrientZ < checkRampeUp) {
    pdebug.println("Rampe! (1)");
    rampeTime = 0;
    return true;
  }
  if(rampeTime < waitRampe) {
    if(getDstSensor(2)<wallBreak) {
      return false;
    } else {
      pdebug.println("Rampe! (2)");
      return true;
    }
  }
  return false;
